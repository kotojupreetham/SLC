...
export function InteractivePipeline() {
  ...
  useEffect(() => {
    ...
    // Record original body overflow to safely restore in cleanup
    originalOverflowRef.current = document.body.style.overflow;

    // Reduced motion shortcut: show all stages in static tab layout without locking scroll or running long animations
    ...
    // Guaranteed cleanup: revert GSAP context, clear timer, restore body overflow
    return () => {
      ...
      {prefersReducedMotion && (
        <div className="py-16 px-6 max-w-6xl mx-auto">
          ...
          <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight mb-4 text-white">
            ...
            <span className="gradient-accent">{SITE_CONTENT.hero.headlineAccent}</span>
          </h1>
          <p className="text-[#94a3b8] text-base mb-8 max-w-2xl">
            {SITE_CONTENT.hero.description}
          </p>

          <h2 className="text-xs font-mono uppercase tracking-widest text-[#38bdf8] mb-4">
            DevOps Lifecycle Stages (Accessible Navigator)
          </h2>

          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8" role="tablist" aria-label="DevOps lifecycle stages">
            {PIPELINE_STAGES.map((s, idx) => (
              <button
                ...
                className={`p-3 rounded-lg border text-left font-mono text-xs transition-colors cursor-pointer ${
                  idx === activeIndex
                    ? "bg-[#0f172a] border-[#38bdf8] text-white font-bold"
                    : "bg-[#090d16] border-[rgba(255,255,255,0.08)] text-[#94a3b8] hover:text-white"
                }`}
              >
                <div className="text-[10px] text-[#64748b]">0{idx + 1} {"//"} STAGE</div>
                <div className="truncate uppercase mt-1">{s.id}</div>
              </button>
            ))}
          </div>

          <div className="p-8 rounded-2xl bg-[#0f172a] border border-[rgba(56,189,248,0.3)] space-y-4">
            <div className="flex items-center gap-3">
              <ActiveIcon className="w-8 h-8 text-[#38bdf8]" />
              <div>
                <MonoLabel className="text-[#38bdf8]">{activeStage.badge}</MonoLabel>
                <h3 className="text-2xl font-bold text-white">{activeStage.title}</h3>
              </div>
            </div>
            <p className="text-sm text-[#94a3b8] leading-relaxed max-w-2xl">{activeStage.description}</p>
            <div className="flex gap-6 border-t border-[rgba(255,255,255,0.1)] pt-4">
              {activeStage.metrics.map((m, i) => (
                <div key={i}>
                  <span className="block text-[10px] font-mono text-[#64748b] uppercase">{m.label}</span>
                  <span className="text-sm font-mono font-bold text-[#38bdf8]">{m.value}</span>
                </div>
              ))}
            </div>
          </div>
