      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            return (
              ...
              >
                <div className="flex flex-col gap-1">
                  ...
                  <span className={cn("text-base font-bold tracking-tight", isSelected ? "text-white" : "text-[#cbd5e1]")}>
                    {service.name}
                  </span>
                </div>
                <div className="flex items-center gap-3">
                  <span
                    className={cn(
                      "text-[10px] font-mono px-3 py-1 rounded-full uppercase tracking-wider",
                      isSelected
                  ...
                  <ArrowRight className={cn("w-4 h-4 transition-transform", isSelected ? "text-[#38bdf8] translate-x-1" : "text-[#475569]")} />
                </div>
              </button>
            );
          })}
        ...
        <div className="lg:col-span-7 glass-panel rounded-3xl p-8 md:p-10 flex flex-col justify-between relative overflow-hidden">
          <div className="relative z-10">
            ...
            <h3 className="text-2xl md:text-3xl font-extrabold text-white mb-3 tracking-tight">{active.name}</h3>
            <p className="text-[#cbd5e1] text-sm md:text-base leading-relaxed mb-8">{active.description}</p>

            <div className="grid grid-cols-2 gap-4 mb-8">
              {active.metrics.map((metric, i) => (
                <div key={i} className="p-4 rounded-2xl bg-[#030712]/70 border border-[rgba(255,255,255,0.08)] shadow-inner">
                  <span className="text-xs font-mono text-[#64748b] block mb-1 uppercase tracking-wider">{metric.label}</span>
                  <span className="text-xl md:text-2xl font-mono font-bold text-[#38bdf8]">{metric.value}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="relative z-10 pt-4 border-t border-[rgba(255,255,255,0.08)]">
            ...
            <div className="flex flex-wrap gap-2">
              {active.tags.map((tag, i) => (
                <span key={i} className="px-3.5 py-1.5 rounded-xl bg-[#1e293b]/80 border border-[#334155] text-xs font-mono text-[#cbd5e1] shadow-sm">
                  {tag}
                </span>
              ))}
            </div>
